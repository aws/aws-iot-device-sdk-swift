...
struct Mqtt5Sample: AsyncParsableCommand {
    ...
    **************************************/    
    ...
    func showMenu() {
        ...
        Commands:
        start - Puts the MQTT5 client in a connected state
        stop - Puts the MQTT5 client in a disconnected state        
                    ...
                    if let qos = getQoS(token: qosToken.lowercased()){
                        Task {
                            do {
                                ...
                                let publishPacket: PublishPacket = PublishPacket(qos: qos, topic: topic, payload: payloadData)
                                ...
                                if let puback: PubackPacket = publishResult.puback {
                                    print("PubackPacket received with reasonCode: \(puback.reasonCode)")
                                }
                            } catch {
                    ...
                    if let qos = getQoS(token: qosToken.lowercased()){
                        Task {
                            do {
                                ...
                                print("SubackPacket received with reasonCode: \(subackPacket.reasonCodes[0])")
                            } catch {
                ...
                // Check if the command begins with "unsubscribe"
                    ...
                    Task {
                        do {
                            ...
                            print("UnsubackPacket received with reason code: \(unsubackPacket.reasonCodes[0])")
                        } catch {
